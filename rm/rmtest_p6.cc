...
RC TEST_RM_PRIVATE_6(const string &tableName)
{
    ...
    // 1. Update tuples
    // 2. Read Attribute
    cout << endl << "***** In RM Test Case Private 6 *****" << endl;
    ...
    for(int i = 0; i < numTuples; i=i+100)
    {
        ...
        if (i % 10000 == 0){
           cout << (i+1) << "/" << numTuples << " records have been processed so far." << endl;
        }
        updateCount++;

        tuples.push_back((char *)tuple);
    }
	cout << "All records have been processed - update count: " << updateCount << endl;
    ...
    cout << "***** RM Test Case Private 6 finished. The result will be examined. *****" << endl << endl;
